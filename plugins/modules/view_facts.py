...
class ViewFacts(BluecatModule):
    def __init__(self):
        self.module_args = dict()

        super(ViewFacts, self).__init__(self.module_args,
                                        supports_check_mode=True,
                                        is_fact=True)

    def exec_module(self, **kwargs):
        results = dict(ansible_facts=dict(views=[]))
        response = self.client.http_get('/views',
        ...
        if response['count'] > 0:
            views = response['data']
            results['ansible_facts']['views'] = views
        return results
