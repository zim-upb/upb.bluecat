...
class ConfigurationFacts(BluecatModule):
    def __init__(self):
        self.module_args = dict()

        super(ConfigurationFacts, self).__init__(self.module_args,
                                           supports_check_mode=True,
                                           is_fact=True)

    def exec_module(self, **kwargs):
        results = dict(ansible_facts=dict(configurations=[]))
        response = self.client.http_get('/configurations',
        ...
        if response['count'] > 0:
            ...
            results['ansible_facts']['configurations'] = configurations
        return results
